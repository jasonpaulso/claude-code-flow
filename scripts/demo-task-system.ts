...
async function runDemo() {
  ...
  const coder = await executor.createAgent("Code Master", [
    "coding",
    "testing",
  ]);
  const analyst = await executor.createAgent("Data Analyst", [
    "analysis",
    "reporting",
  ]);
  const researcher = await executor.createAgent("Research Bot", [
    "research",
    "documentation",
  ]);
  ...
  const tasks = [
    await executor.createTask("code", "Implement user authentication", {
      feature: "auth",
    }),
    await executor.createTask("analyze", "Analyze user behavior data", {
      dataset: "users",
    }),
    ...
    await executor.createTask("code", "Add unit tests for payment module", {
      module: "payments",
    }),
    await executor.createTask("analyze", "Generate performance report", {
      period: "Q4",
    }),
  ];

...
async function testCoordinationFeatures() {
  ...
  try {
    ...
    const acquire2Promise = coordinator.acquireResource(
      "resource-A",
      "agent-2",
    );
    ...
    await coordinator.reportConflict("resource", "shared-resource", [
      "agent-1",
      "agent-2",
    ]);
    ...
    await coordinator.reportConflict("task", "disputed-task", [
      "agent-1",
      "agent-2",
    ]);
    ...
    console.log(
      `  Advanced scheduling: ${metrics.advancedScheduling ? "Enabled" : "Disabled"}`,
    );

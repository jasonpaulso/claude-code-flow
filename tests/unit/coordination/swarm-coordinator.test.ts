...
import { SwarmCoordinator, SwarmAgent, SwarmTask, SwarmObjective } from '../../../src/coordination/swarm-coordinator.ts';
...
describe('SwarmCoordinator', () => {
  ...
  describe('Agent Management', () => {
    ...
    it('should track agent metrics', async () => {
      const agentId = await coordinator.registerAgent('MetricsAgent', 'analyzer');
      const agent = coordinator.getAgentStatus(agentId);
  ...
  describe('Claude Process Integration', () => {
    ...
    it('should build correct Claude arguments', async () => {
      const objectiveId = await coordinator.createObjective('Test Claude args', 'development');
      const objective = coordinator.getObjectiveStatus(objectiveId);
      const task = objective!.tasks[0];
    ...
    it('should select appropriate tools for agent types', async () => {
      const researchers = await coordinator.registerAgent('Researcher', 'researcher');
      const developer = await coordinator.registerAgent('Developer', 'developer');

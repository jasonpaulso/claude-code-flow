...
describe("CLI Commands - Comprehensive Tests", () => {
          ...
          switch (args.action) {
            case "create":
              ...
              if (args.capabilities)
                cli.stdout(`Capabilities: ${args.capabilities}`);
        ...
        options: [
          {
            ...
            type: "string",
            required: true,
            description: "Action: run, list, cancel",
          },
          {
            name: "command",
            type: "string",
        ...
        handler: async (args) => {
          switch (args.action) {
            ...
            case "list":
              ...
              cli.stdout(
                '- task-001: Running command "echo hello" (agent: agent-1)',
              );
              cli.stdout(
                '- task-002: Completed command "ls -la" (agent: agent-2)',
              );
              break;
  ...
  describe("Memory Command", () => {
    ...
    it("should set a memory entry", async () => {
      ...
      assertStringIncludes(
        output.stdout.join(" "),
        "Setting memory: default:test-key",
      );
    ...
    it("should set a memory entry with namespace and tags", async () => {
      ...
      assertStringIncludes(
        output.stdout.join(" "),
        "Setting memory: project:project-key",
      );
    ...
    it("should retrieve a memory entry", async () => {
      ...
      assertStringIncludes(
        output.stdout.join(" "),
        "Retrieving memory: default:existing-key",
      );
      assertStringIncludes(
        output.stdout.join(" "),
        "Value: example-value-for-existing-key",
      );
    });
    ...
    it("should delete a memory entry", async () => {
      ...
      assertStringIncludes(
        output.stdout.join(" "),
        "Deleting memory: default:old-key",
      );
      assertStringIncludes(output.stdout.join(" "), "deleted successfully");
    });

    it("should require key and value for set action", async () => {
      const exitCode1 = await cli.execute([
        "memory",
        "--action",
        "set",
        "--key",
      ...
      const exitCode2 = await cli.execute([
        "memory",
        "--action",
        "set",
        "--value",
  ...
  describe("CLI Integration Scenarios", () => {
    ...
    it("should handle concurrent CLI operations", async () => {
      ...
      const promises = concurrentCommands.map(async (command, index) => {
        ...
        concurrentCLI.command({
          ...
          handler: async (args) => {
            ...
            concurrentCLI.stdout(
              `Agent ${args.action} ${args.name || ""} (concurrent-${index})`,
            );
            return 0;
          },
        });
        ...
        concurrentCLI.command({
          ...
          handler: async (args) => {
            await AsyncTestUtils.delay(Math.random() * 100); // Simulate processing time
            concurrentCLI.stdout(
              `Task ${args.action} ${args.command || ""} (concurrent-${index})`,
            );
            return 0;
          },
        });
      ...
      console.log(
        `${results.length} concurrent CLI operations completed successfully`,
      );
    });
    ...
    it("should handle error recovery in CLI workflows", async () => {
      ...
      for (const [index, command] of errorWorkflow.entries()) {
        ...
        console.log(
          `Step ${index + 1}: ${command.join(" ")} -> exit code ${exitCode}`,
        );
      }
    ...
    beforeEach(() => {
      cli.command({
        ...
        handler: async (args) => {
          ...
          cli.stdout(
            `Performance test completed: ${args.iterations} iterations`,
          );
          return 0;
        },
      });

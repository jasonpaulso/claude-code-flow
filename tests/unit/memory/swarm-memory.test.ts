...
import { assertEquals, assertExists, assertThrows, assertArrayIncludes } from '@std/assert';
import { afterEach, beforeEach, describe, it } from '@std/testing/bdd';
import { SwarmMemoryManager, SwarmMemoryEntry } from '../../../src/memory/swarm-memory.ts';
...
describe('SwarmMemoryManager', () => {
  ...
  describe('Memory Entry Management', () => {
    ...
    it('should validate memory entry data', async () => {
      ...
      // Test invalid type
      await assertThrows(
        async () => await memoryManager.remember(testAgentId, 'invalid' as any, 'content'),
        Error,
      ...
      // Test null content
      await assertThrows(
        async () => await memoryManager.remember(testAgentId, 'knowledge', null),
        Error,
      ...
      await assertThrows(
        async () => await memoryManager.remember(
          testAgentId, 
          'knowledge', 
          'content',
          { priority: 15 }
    ...
    it('should enforce content size limits', async () => {
      ...
      await assertThrows(
        async () => await memoryManager.remember(testAgentId, 'knowledge', largeContent),
        Error,
  ...
  describe('Memory Querying', () => {
    beforeEach(async () => {
      // Add test data
      await memoryManager.remember(testAgentId, 'knowledge', { topic: 'AI' }, { 
        tags: ['ai', 'ml'], 
        taskId: 'task-1' 
      });
      await memoryManager.remember(testAgentId, 'result', { outcome: 'success' }, { 
        tags: ['success'], 
        taskId: 'task-2' 
      });
      await memoryManager.remember('other-agent', 'knowledge', { topic: 'web' }, { 
        tags: ['web'] 
      });
    });
  ...
  describe('Memory Sharing', () => {
    ...
    it('should share memory between agents', async () => {
      ...
      const sharedEntries = await memoryManager.recall({ agentId: targetAgent });
    ...
    it('should prevent sharing private memories', async () => {
      ...
      await assertThrows(
        async () => await memoryManager.shareMemory(privateEntryId, targetAgent),
        Error,
  ...
  describe('Knowledge Base Management', () => {
    ...
    it('should search knowledge base content', async () => {
      ...
      await memoryManager.remember(
        testAgentId,
        'knowledge',
        { topic: 'HTML forms', content: 'Forms are essential for web interaction' },
        { tags: ['html', 'forms'] }
      );
  ...
  describe('Memory Import/Export', () => {
    beforeEach(async () => {
      // Add test data
      await memoryManager.remember(testAgentId, 'knowledge', { data: 'export test' });
  ...
  describe('Agent Memory Snapshots', () => {
    beforeEach(async () => {
      ...
      await memoryManager.remember(testAgentId, 'result', { result: 'success' }, { shareLevel: 'public' });
    });
    ...
    it('should handle empty agent snapshots', async () => {
      const snapshot = await memoryManager.getAgentMemorySnapshot('non-existent-agent');

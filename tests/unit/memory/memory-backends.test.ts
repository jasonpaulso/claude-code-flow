  ...
  describe("SQLite Memory Backend", () => {
    ...
    describe("Querying and Search", () => {
      ...
      it("should search by tags", async () => {
        ...
        await backend.store("search", "tagged-1", { data: 1 }, [
          "important",
          "urgent",
        ]);
      ...
      it("should search with pagination", async () => {
        ...
        const intersection = new Set(
          [...page1Keys].filter((k) => page2Keys.has(k)),
        );
    ...
    describe("Performance and Scalability", () => {
      it("should handle bulk operations efficiently", async () => {
        ...
        console.log(
          `SQLite bulk write performance: ${stats.mean.toFixed(2)}ms average`,
        );
      });
      ...
      it("should handle concurrent operations", async () => {
        ...
        await Promise.all(
          operations.map((op) => backend.store(op.namespace, op.key, op.value)),
        );
      ...
      it("should maintain performance with large datasets", async () => {
        ...
        for (let i = 0; i < largeEntries.length; i += 100) {
          ...
          await Promise.all(
            batch.map((entry) => backend.store("large", entry.id, entry)),
          );
        }
        ...
        console.log(
          `SQLite large dataset search: ${stats.mean.toFixed(2)}ms average`,
        );
      });
        ...
        for (const [category, values] of Object.entries(edgeCases)) {
          for (const [name, value] of Object.entries(values)) {
            try {
              ...
              console.log(
                `Edge case ${category}-${name} failed: ${error.message}`,
              );
            }
          }
        }
...
`;
        ...
        const documents = [
          {
            ...
            value: {
              ...
              content:
                "Learn JavaScript programming with examples and best practices.",
    ...
    describe("Performance and File Management", () => {
      it("should handle many files efficiently", async () => {
        ...
        console.log(
          `Markdown backend performance: ${stats.mean.toFixed(2)}ms average`,
        );
      });
      ...
      it("should handle directory cleanup", async () => {
        ...
        console.log(
          `Directory cleanup: ${dirExists ? "preserved" : "cleaned"}`,
        );
      });
    });
  ...
  describe("Backend Comparison and Compatibility", () => {
    ...
    it("should handle performance comparison", async () => {
      ...
      const sqliteStats = await PerformanceTestUtils.benchmark(
        async () => {
          const entry =
            testEntries[Math.floor(Math.random() * testEntries.length)];
      ...
      console.log(
        `Markdown backend: ${markdownStats.mean.toFixed(2)}ms average`,
      );

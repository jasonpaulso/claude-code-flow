...
describe("Orchestrator - Enhanced Tests", () => {
  ...
  describe("Initialization", () => {
    it("should initialize all components in correct order", async () => {
      ...
      const initEvent = events.find(
        (e) => e.event === "orchestrator.initialized",
      );
  ...
  describe("Task Management", () => {
    ...
    it("should create and execute tasks correctly", async () => {
      ...
      const result = await orchestrator.createAndExecuteTask(
        agentProfile,
        task,
      );
  ...
  describe("Health Monitoring", () => {
    ...
    it("should detect unhealthy components", async () => {
      ...
      assertEquals(
        health.components.terminal.error,
        "Terminal manager is down",
      );
    });
  ...
  describe("Memory Management", () => {
    ...
    it("should manage agent memory banks correctly", async () => {
      ...
      await orchestrator.storeMemory(bankId, "test-key", {
        data: "test-value",
      });
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should recover from temporary component failures", async () => {
      ...
      mocks.terminalManager.sendCommand = spy(
        async (terminalId: string, command: any) => {
          failureCount++;
          if (failureCount <= 3) {
            throw new Error("Temporary failure");
          }
  ...
  describe("Shutdown and Cleanup", () => {
    it("should shutdown gracefully", async () => {
      ...
      const shutdownEvent = events.find(
        (e) => e.event === "orchestrator.shutdown",
      );

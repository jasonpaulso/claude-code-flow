...
describe("EventBus", () => {
  ...
  it("should wait for event", async () => {
    ...
    setTimeout(() => {
      eventBus.emit(SystemEvents.SYSTEM_READY as any, {
        timestamp: new Date(),
      });
    }, 100);
  ...
  it("should filter events", () => {
    ...
    eventBus.emit(SystemEvents.TASK_COMPLETED as any, {
      taskId: "task-1",
      result: "success",
    });
    eventBus.emit(SystemEvents.TASK_COMPLETED as any, {
  ...
  it("should remove event listeners", () => {
    ...
    eventBus.emit(SystemEvents.AGENT_ERROR as any, {
      agentId: "test",
  ...
  it("should track event statistics", () => {
    ...
    const taskCreatedStat = stats.find(
      (s) => s.event === SystemEvents.TASK_CREATED,
    );
    const taskCompletedStat = stats.find(
      (s) => s.event === SystemEvents.TASK_COMPLETED,
    );

...
describe("EventBus - Enhanced Tests", () => {
  ...
  describe("Performance and Scalability", () => {
    it("should handle high-frequency events efficiently", async () => {
      ...
      console.log(
        `High-frequency event performance: ${stats.mean.toFixed(4)}ms per event`,
      );
    });
    ...
    it("should handle many concurrent handlers efficiently", async () => {
      ...
      console.log(
        `Many handlers performance: ${stats.mean.toFixed(2)}ms for ${handlerCount} handlers`,
      );
    });
    ...
    it("should handle load testing scenario", async () => {
      ...
      console.log(
        `Load test: ${results.successfulRequests} events/sec, avg=${results.averageResponseTime.toFixed(3)}ms`,
      );
    });
  });
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should handle handler that modifies handler list", () => {
      ...
      const totalCalls = handlers.reduce(
        (sum, handler) => sum + handler.calls.length,
        0,
      );

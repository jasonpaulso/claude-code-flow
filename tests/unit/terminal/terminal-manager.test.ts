...
describe("Terminal Manager - Comprehensive Tests", () => {
  ...
  beforeEach(() => {
    ...
    mockPool = MockFactory.createMock({
      ...
      executeCommand: async (sessionId: string, command: any) =>
        `Output for: ${command}`,
      listSessions: () => [],
      getSessionCount: () => 0,
      getAvailableSlots: () => 5,
  ...
  describe("Performance and Scalability", () => {
    ...
    it("should handle high session creation throughput", async () => {
      ...
      console.log(
        `Session creation performance: ${stats.mean.toFixed(2)}ms average`,
      );
    });
    ...
    it("should handle high command execution throughput", async () => {
      ...
      console.log(
        `Command execution performance: ${stats.mean.toFixed(2)}ms average`,
      );
    });
    ...
    it("should handle memory efficiently with many sessions", async () => {
      const { leaked } = await MemoryTestUtils.checkMemoryLeak(async () => {
        ...
        await Promise.all(
          sessionIds.map((sessionId) =>
            terminalManager.terminateTerminal(sessionId),
          ),
        );
      });
    ...
    it("should maintain performance under resource pressure", async () => {
      ...
      const { stats } = await PerformanceTestUtils.benchmark(
        () => {
          ...
          return terminalManager.sendCommand(
            randomSessionId,
            'echo "Under pressure"',
          );
        },
      ...
      console.log(
        `Performance under pressure: ${stats.mean.toFixed(2)}ms average`,
      );
    });
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should recover from temporary adapter failures", async () => {
      ...
      try {
        const result = await terminalManager.sendCommand(
          sessionId,
          "test-command",
        );
  ...
  describe("Shutdown and Cleanup", () => {
    ...
    it("should handle shutdown with active sessions", async () => {
      ...
      const commandPromise = terminalManager.sendCommand(
        sessionId,

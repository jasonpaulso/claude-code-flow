...
describe("Start Command E2E", () => {
  describe("JavaScript CLI", () => {
    it("should show help with UI option", async () => {
      ...
      assertEquals(
        output.includes("Launch interactive process management UI"),
        true,
      );
    ...
    it("should handle UI flag", async () => {
      ...
      assertEquals(success, true);
      assertEquals(
        output.includes("Launching interactive process management UI"),
        true,
      );
    });
  });
      ...
      try {
        ...
        const command = new Deno.Command(Deno.execPath(), {
          args: [
            "run",
            "--allow-all",
            originalCwd + "/src/cli/simple-cli.js",
            "start",
            "--daemon",
          ],
          stdout: "piped",
        ...
        const pidExists = await Deno.stat(".claude-flow.pid")
          .then(() => true)
          .catch(() => false);

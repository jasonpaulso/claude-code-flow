...
describe('Swarm E2E Tests', () => {
  ...
  describe('CLI Command Tests', () => {
    ...
    it('should create and execute swarm objective with dry-run', async () => {
      const cmd = new Deno.Command('./bin/claude-flow', {
        args: [
          'swarm',
          'new',
    ...
    it('should list available swarm strategies', async () => {
      const cmd = new Deno.Command('./bin/claude-flow', {
      ...
      assertEquals(code, 0, `Strategies command failed with stderr: ${errorOutput}`);
    ...
    it('should show swarm status when no swarms are running', async () => {
      const cmd = new Deno.Command('./bin/claude-flow', {
        args: ['swarm', 'status'],
        stdout: 'piped',
        stderr: 'piped'
      ...
      assertEquals(code, 0, `Status command failed with stderr: ${errorOutput}`);
  ...
  describe('Swarm Workflow Tests', () => {
    it('should complete a simple research workflow', async () => {
      ...
      assertEquals(code, 0, `Research workflow failed with stderr: ${errorOutput}`);
    ...
    it('should handle development workflow with multiple agents', async () => {
      const cmd = new Deno.Command('./bin/claude-flow', {
        args: [
          'swarm',
          'new',
      ...
      assertEquals(code, 0, `Development workflow failed with stderr: ${errorOutput}`);
    ...
    it('should validate input parameters', async () => {
      ...
      const { code: invalidCode, stderr: invalidStderr } = await invalidStrategyCmd.output();
      ...
      const { code: agentsCode, stderr: agentsStderr } = await invalidAgentsCmd.output();
  ...
  describe('Memory and State Management', () => {
    it('should persist swarm state between runs', async () => {
      ...
      const { code: statusCode, stdout: statusStdout } = await statusCmd.output();
    ...
    it('should handle memory export and import', async () => {
      ...
      const exportCmd = new Deno.Command('./bin/claude-flow', {
        args: ['memory', 'export', '--output', './tests/temp/memory-export.json'],
        stdout: 'piped',
        stderr: 'piped'
      });

      const { code: exportCode, stdout: exportStdout } = await exportCmd.output();
    ...
    it('should validate configuration files', async () => {
      ...
      const cmd = new Deno.Command('./bin/claude-flow', {
        args: [
          ...
          'new',
          'Config test',
          '--config', './tests/temp/test-config.json',
          '--dry-run'
        ],
        stdout: 'piped',
        stderr: 'piped'

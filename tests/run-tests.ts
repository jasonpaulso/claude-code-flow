  ...
  try {
    ...
    for (const testSuite of suitesToRun) {
      ...
      const result = await runTestSuite(testSuite, {
        ...
        verbose,
        parallel,
        timeout: timeout || TEST_CONFIG.timeout[testSuite as keyof typeof TEST_CONFIG.timeout]
      });
      ...
      if (result.failed > 0) {
        console.log(`❌ ${testSuite} tests: ${result.passed} passed, ${result.failed} failed`);
      } else {
...
async function runTestSuite(
  ...
  const testDir = TEST_CONFIG.directories[suite as keyof typeof TEST_CONFIG.directories];
...
async function checkCoverageThresholds(): Promise<boolean> {
  ...
  try {
    ...
    console.log(`Coverage: ${totalCoverage.toFixed(2)}% (threshold: ${threshold}%)`);

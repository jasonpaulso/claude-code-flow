...
describe("Orchestrator-Terminal Integration", () => {
  ...
  describe("agent spawning and terminal integration", () => {
    ...
    it("should handle concurrent tasks across multiple agents", async () => {
      ...
      results.forEach((result, index) => {
        ...
        assertEquals(result.output.stdout.includes(`task ${index + 1}`), true);
      });
    });
  ...
  describe("error handling and recovery", () => {
    ...
    it("should retry failed tasks", async () => {
      ...
      const originalExecute =
        terminalManager.executeCommand.bind(terminalManager);
  ...
  describe("memory integration", () => {
    it("should persist agent session data", async () => {
      ...
      const storedEntries = memoryManager.storeEntry.calls.map(
        (call) => call.args[0],
      );
      const spawnEntry = storedEntries.find(
        (entry) =>

...
const testAgentProfiles: AgentProfile[] = [
  ...
  },
  {
    ...
    systemPrompt:
      "You are an implementation agent focusing on code development.",
  ...
  for (let i = 0; i < count; i++) {
    ...
    const task: Task = {
      ...
      dependencies: hasDependency
        ? [`task-${Math.max(1, i - Math.floor(Math.random() * 5))}`]
        : [],
      status: "pending",
      input: {
  ...
  try {
    ...
    const spawnPromises = testAgentProfiles.map(async (profile) => {
      try {
        ...
        console.log(
          `✅ Spawned agent: ${profile.name} (Session: ${sessionId})`,
        );
        return sessionId;
      } catch (error) {
    ...
    console.log(
      `  Average task duration: ${Math.round(finalMetrics.avgTaskDuration)}ms`,
    );
    console.log(
      `  Memory usage: ${Math.round(finalMetrics.memoryUsage.heapUsed / 1024 / 1024)}MB`,
    );

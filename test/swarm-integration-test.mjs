...
class SwarmIntegrationTester {
  ...
  async testDevelopmentStrategy() {
    ...
    try {
      ...
      agents.push(
        await coordinator.registerAgent("Documenter-1", "documenter", {
          documentation: true,
        }),
      );
      ...
      // Wait for completion
      const result = await this.waitForObjectiveCompletion(
        coordinator,
        objectiveId,
      );
      ...
      for (const task of tasks) {
        if (task.result && task.result.artifacts) {
          filesCreated = true;
          console.log(
            `  ✅ Task "${task.name}" created artifacts:`,
            Object.keys(task.result.artifacts),
          );
        }
      }
  ...
  async testResearchStrategy() {
    ...
    try {
      ...
      assert(
        ...
        "Research objective not completed",
      );
  ...
  async testAnalysisStrategy() {
    ...
    try {
      ...
      const analysisTasks = tasks.filter(
        (t) => t.type === "analysis" || t.type === "generic",
      );
  ...
  async testCoordinationModes() {
    ...
    for (const mode of modes) {
      ...
      try {
        ...
        assert(
          ["executing", "completed"].includes(status),
  ...
  async testBackgroundExecution() {
    ...
    try {
      ...
      assert(
        result.output.includes("Hello from background"),
        "Output mismatch",
      );
  ...
  // Helper methods
  async waitForObjectiveCompletion(
    coordinator,
    objectiveId,
    ...
    while (Date.now() - startTime < timeout) {
      ...
      process.stdout.write(
        `\r  ⏳ Progress: ${status.tasks.completed}/${status.tasks.total} tasks`,
      );
  ...
  reportResults() {
    ...
    for (const test of this.results.tests) {
      ...
      console.log(
        `  ${icon} ${test.name}${test.error ? ` - ${test.error}` : ""}`,
      );
    }

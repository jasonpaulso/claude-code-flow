...
export class MessageBus extends EventEmitter {
  ...
  async joinChannel(channelId: string, agentId: AgentId): Promise<void> {
    ...
    if (!this.canJoinChannel(channel, agentId)) {
      throw new Error(
        `Agent ${agentId.id} not allowed to join channel ${channelId}`,
      );
    }
  ...
  async dequeueMessage(
    ...
    if (!subscriber) {
      throw new Error(
        `Subscriber ${subscriberId} not found in queue ${queueId}`,
      );
    }
  ...
  private getDefaultBroadcastChannel(): MessageChannel | undefined {
    return Array.from(this.channels.values()).find(
      (channel) => channel.type === "broadcast",
    );
  }
  ...
  getMetrics(): any {
    const avgLatency =
      this.deliveryLatencies.length > 0
        ? this.deliveryLatencies.reduce((sum, lat) => sum + lat, 0) /
          this.deliveryLatencies.length
        : 0;
    ...
    return {
      ...
      successRate:
        this.messagesSent > 0
          ? (this.messagesDelivered / this.messagesSent) * 100
          : 100,
    };
  }
}

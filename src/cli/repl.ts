...
export async function startREPL(options: any = {}): Promise<void> {
  ...
  const commands: REPLCommand[] = [
    ...
    },
    {
      ...
      handler: async (args) => {
        ...
        recent.forEach((cmd, i) => {
          ...
          console.log(
            `${colors.gray(lineNumber.toString().padStart(3))} ${cmd}`,
          );
        });
      },
    },
    ...
    },
    {
      ...
      handler: async (args, ctx) => {
        ...
        try {
          const newDir =
            args[0] === "~" ? Deno.env.get("HOME") || "/" : args[0];
          Deno.chdir(newDir);
  ...
  if (!options.quiet) {
    ...
    console.log(
      colors.gray('Type "help" for available commands or "exit" to quit.\n'),
    );
  }
    ...
    try {
      ...
      if (command) {
        ...
        if (suggestions.length > 0) {
          console.log(
            colors.gray("Did you mean:"),
            suggestions.map((s) => colors.cyan(s)).join(", "),
          );
        }
      }
    } catch (error) {
...
async function showSystemStatus(
  ...
  console.log(
    `${colors.white("Working Directory:")} ${context.workingDirectory}`,
  );
  console.log(
    `${colors.white("Last Activity:")} ${context.lastActivity.toLocaleTimeString()}`,
  );

  if (context.currentSession) {
    console.log(
      `${colors.white("Current Session:")} ${context.currentSession}`,
    );
  }

  console.log(
    `${colors.white("Commands in History:")} ${context.history.length}`,
  );
...
async function connectToOrchestrator(
  ...
  if (success) {
    ...
    console.log(
      colors.gray("Make sure Claude-Flow is running with: claude-flow start"),
    );
  }
}
...
async function handleAgentCommand(
  ...
  if (args.length === 0) {
    console.log(
      colors.gray("Usage: agent <spawn|list|terminate|info> [options]"),
    );
    return;
  }
...
async function handleAgentSpawn(args: string[]): Promise<void> {
  if (args.length === 0) {
    ...
    console.log(
      colors.gray(
        "Types: coordinator, researcher, implementer, analyst, custom",
      ),
    );
    return;
  }
...
async function showAgentInfo(agentId: string): Promise<void> {
  ...
  console.log(
    `${colors.white("Status:")} ${formatStatusIndicator("success")} active`,
  );
...
async function handleTaskCommand(
  ...
  if (args.length === 0) {
    console.log(
      colors.gray("Usage: task <create|list|status|cancel> [options]"),
    );
    return;
  }
...
async function showTaskStatus(taskId: string): Promise<void> {
  ...
  console.log(
    `${colors.white("Status:")} ${formatStatusIndicator("running")} running`,
  );
  console.log(
    `${colors.white("Progress:")} ${formatProgressBar(65, 100, 20)} 65%`,
  );
...
async function handleMemoryCommand(
  ...
  switch (subcommand) {
    ...
    case "query":
      console.log(
        colors.yellow("Memory query functionality not yet implemented in REPL"),
      );
      break;
    case "export":
      console.log(
        colors.yellow(
          "Memory export functionality not yet implemented in REPL",
        ),
      );
      break;
    default:
...
async function showWorkflowStatus(workflowId: string): Promise<void> {
  ...
  console.log(
    `${colors.white("Progress:")} ${formatProgressBar(75, 100, 20)} 75%`,
  );

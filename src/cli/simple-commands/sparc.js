...
async function runSparcMode(subArgs, flags) {
  ...
  try {
    ...
    const enhancedTask = createSparcPrompt(
      mode,
      taskDescription,
      memoryNamespace,
    );
    ...
    const isNonInteractive =
      subArgs.includes("--non-interactive") || subArgs.includes("-n");
    const enablePermissions = subArgs.includes("--enable-permissions");
    ...
    if (isNonInteractive) {
      ...
      console.log(
        `First 200 chars of prompt: ${enhancedTask.substring(0, 200)}...`,
      );
    }
    console.log();
...
async function executeClaude(
  ...
  if (
    ...
    console.log(
      ...
      enablePermissions
        ? `Specified: ${toolsList}`
        : "ALL tools enabled (MCP, WebSearch, etc.)",
    );
    console.log(
      "Mode:",
      isNonInteractive ? "🤖 Non-interactive" : "💬 Interactive",
    );
    console.log("Args array length:", claudeArgs.length);
    console.log(
      "First arg (prompt) length:",
      claudeArgs[0].length,
      "characters",
    );
  ...
  try {
    ...
    if (status.success) {
      ...
      printError(
        `SPARC instance ${instanceId} exited with code ${status.code}`,
      );
    }
  } catch (err) {
...
function showSparcHelp() {
  ...
  console.log(
    "  modes                    List available SPARC development modes",
  );
  console.log(
    "  info <mode>              Show detailed information about a mode",
  );
  console.log(
    "  run <mode> <task>        Execute a task in specified SPARC mode",
  );
  console.log(
    "  tdd <task>               Run Test-Driven Development workflow",
  );
  console.log();
  console.log("Examples:");
  console.log(
    '  claude-flow sparc "orchestrate app development"    # Uses sparc orchestrator',
  );
  ...
  console.log(
    '  claude-flow sparc run code "add login feature" --non-interactive',
  );
  console.log(
    '  claude-flow sparc run tdd "create test suite" --namespace tests',
  );
  console.log(
    '  claude-flow sparc tdd "payment processing system" --interactive',
  );
  console.log();
  ...
  console.log(
    "  --non-interactive, -n    Run in non-interactive mode with stream-json output",
  );
  console.log(
    "  --enable-permissions     Enable permission prompts (default: skip permissions)",
  );
  console.log(
    "  --namespace <ns>         Use custom memory namespace (default: mode slug)",
  );
  console.log("  --config <path>          Use custom MCP configuration file");
  console.log();
  console.log("Permission Behavior:");
  console.log(
    "  By default, SPARC runs with --dangerously-skip-permissions for efficiency",
  );
  console.log(
    "  Use --enable-permissions to restore permission prompts if needed",
  );
  console.log();
  console.log("Non-Interactive Mode:");
  console.log("  When using --non-interactive, claude will be executed with:");
  console.log(
    "  - --dangerously-skip-permissions (unless --enable-permissions is set)",
  );
  ...
  console.log(
    "  A cyclical orchestration where outputs from one phase feed into the next:",
  );

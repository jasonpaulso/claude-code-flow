  ...
  },
  {
    ...
    examples: [
      ...
      },
      {
        ...
        explanation:
          "Shows all currently running agents with their status and task counts",
      },
      {
    ...
    examples: [
      {
        ...
        command:
          'claude-flow task create research "Find papers on quantum computing" --priority 5',
        explanation:
          "Creates a high-priority research task with specific instructions",
      },
      {
        ...
        explanation:
          "Creates a task that waits for task-001 to complete before starting",
      },
      {
        description: "Assign task to specific agent",
        command:
          'claude-flow task create implementation "Write API client" --assign agent-003',
      ...
      },
      {
        ...
        explanation:
          "Shows detailed status and progress information for a task",
      },
      {
        description: "Cancel a running task",
        command:
          'claude-flow task cancel task-001 --reason "Requirements changed"',
  ...
  },
  {
    ...
    examples: [
      ...
      },
      {
        ...
        explanation:
          "Runs Claude with --dangerously-skip-permissions flag to avoid interruptions",
      },
      {
        description: "Spawn Claude with custom tools",
        command:
          'claude-flow claude spawn "analyze codebase" --tools "View,Edit,GrepTool,LS"',
        explanation:
          "Specifies exactly which tools Claude can use for the task",
      },
      {
        description: "Spawn Claude with test coverage target",
        command:
          'claude-flow claude spawn "write unit tests" --coverage 95 --commit feature',
        explanation:
          "Sets test coverage goal to 95% and commits after each feature",
      },
      {
        description: "Dry run to preview command",
        command:
          'claude-flow claude spawn "build API" --mode backend-only --dry-run',
        explanation:
          "Shows what would be executed without actually running Claude",
      },
    ],
    tutorial: [
    ...
    examples: [
      {
        ...
        explanation:
          "Executes a workflow definition and monitors progress in real-time",
      },
      {
        description: "Validate workflow before running",
        command: "claude-flow workflow validate my-workflow.json --strict",
        explanation:
          "Checks workflow syntax and dependencies without executing",
      },
      {
        description: "Generate workflow template",
        command:
          "claude-flow workflow template research --output research-workflow.json",
        explanation:
          "Creates a pre-configured workflow template for research tasks",
      },
      {
    ...
    examples: [
      {
        ...
        explanation:
          "Continuously monitors system health and updates every few seconds",
      },
      {
  ...
  },
  {
    ...
    examples: [
      ...
      },
      {
        ...
        explanation:
          "Shows all saved sessions with creation dates and metadata",
      },
      {
      ...
      },
      {
        ...
        command:
          "claude-flow session export session-001 backup.json --include-memory",
      ...
      },
      {
        ...
        explanation:
          "Shows what sessions would be deleted (older than 30 days)",
      },
    ],
    tutorial: [
...
function showMainHelp(): void {
  ...
  console.log(
    colors.white("Claude-Flow is an advanced AI agent orchestration system."),
  );
  console.log(
    colors.white(
  ...
  console.log(
    colors.gray("  claude-flow help getting-started    # Beginner tutorial"),
  );
  console.log(
    colors.gray(
  ...
  console.log(
    colors.gray("  claude-flow help <topic>            # Specific topic help"),
  );
  console.log();
  ...
  for (const [category, description] of Object.entries(categories)) {
    ...
    for (const topic of topics) {
      console.log(
        colors.gray(`    ${topic.name.padEnd(20)} ${topic.description}`),
      );
    }
  }
  ...
  console.log();
  console.log(
    colors.gray('Use "claude-flow help <topic>" for detailed information.'),
  );
...
async function showTopicHelp(topicName: string, options: any): Promise<void> {
  ...
  if (!topic) {
    ...
    if (similar.length > 0) {
      ...
      console.log(
        colors.gray('Use "claude-flow help --all" to see all topics.'),
      );
    }
    return;
  }
  ...
  if (!options.examples && !options.tutorial) {
    ...
    if (topic.examples) {
      ...
      if (topic.examples.length > 3) {
        console.log(
          colors.gray(`    ... and ${topic.examples.length - 3} more`),
        );
      }
      console.log();

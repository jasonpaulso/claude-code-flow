...
export const claudeCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option("--coverage <coverage:number>", "Test coverage target", {
        default: 80,
      })
      .option(
        ...
        try {
          ...
          claude.on("exit", (code) => {
            if (code === 0) {
              ...
              console.log(
                colors.red(
                  `Claude instance ${instanceId} exited with code ${code}`,
                ),
              );
            }
          });
          ...
          console.error(
            colors.red("Failed to spawn Claude:"),
    ...
    new Command()
      ...
      .action(async (options: any, workflowFile: string) => {
        try {
          ...
          console.error(
            colors.red("Failed to process workflow:"),
            (error as Error).message,
          );
        }
      }),

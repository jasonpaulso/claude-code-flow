...
export const mcpCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option("-p, --port <port:number>", "Port for MCP server", {
        default: 3000,
      })
      ...
      .action(async (options: any) => {
        try {
          ...
          console.log(
            colors.cyan(
              `📡 Server URL: http://${options.host}:${options.port}`,
            ),
          );
          console.log(
            colors.cyan(`🔧 Available tools: Research, Code, Terminal, Memory`),
          );
          console.log(
            colors.cyan(
  ...
  .command(
    ...
    new Command().description("Show MCP server status").action(async () => {
      try {
        ...
        console.log(
          `🌐 Status: ${isRunning ? colors.green("Running") : colors.red("Stopped")}`,
        );

        if (isRunning) {
          console.log(`📍 Address: ${config.mcp.host}:${config.mcp.port}`);
          console.log(
            `🔐 Authentication: ${config.mcp.auth ? colors.green("Enabled") : colors.yellow("Disabled")}`,
          );
          console.log(`🔧 Tools: ${colors.green("Available")}`);
          console.log(`📊 Metrics: ${colors.green("Collecting")}`);
        } else {
          console.log(
            colors.gray('Use "claude-flow mcp start" to start the server'),
          );
        }
      } catch (error) {
        console.error(
  ...
  .command(
    ...
    new Command()
      ...
      .option("-n, --lines <lines:number>", "Number of log lines to show", {
        default: 50,
      })
      .action((options: any) => {
        console.log(
          colors.cyan(`MCP Server Logs (last ${options.lines} lines):`),
        );

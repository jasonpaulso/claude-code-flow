  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      try {
        ...
        const minimal =
          (ctx.flags.minimal as boolean) || (ctx.flags.m as boolean);
        ...
        console.log(
          "2. Run 'npx claude-flow start' to begin the orchestration system",
        );
        console.log(
          "3. Use 'claude --dangerously-skip-permissions' for unattended operation",
        );
        console.log(
        ...
        case "create": {
          ...
          try {
            ...
            await persist.saveTask({
              ...
              dependencies: ctx.flags.deps
                ? (ctx.flags.deps as string).split(",")
                : [],
              metadata: {},
              progress: 0,
              createdAt: Date.now(),
        ...
        case "workflow": {
          ...
          try {
            ...
            if (ctx.flags.execute) {
              warning(
                "Workflow execution would start here (not yet implemented)",
              );
        ...
        case "spawn": {
          ...
          try {
            ...
            await persist.saveAgent({
              ...
              systemPrompt:
                (ctx.flags.prompt as string) || getDefaultPromptForType(type),
  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      try {
        ...
        const isRunning = await Deno.stat("orchestrator.log")
          .then(() => true)
          .catch(() => false);
        ...
        console.log(
          `🤖 Agents: ${stats.activeAgents} active (${stats.totalAgents} total)`,
        );
        console.log(
          `📋 Tasks: ${stats.pendingTasks} in queue (${stats.totalTasks} total)`,
        );
      ...
      switch (subcommand) {
        case "start": {
          ...
          try {
            ...
            if (!health.healthy) {
              warning(
                "Orchestrator is not running. Start it first with 'claude-flow start'",
              );
              return;
            }

            success(
              `MCP server is running as part of the orchestration system`,
            );
            console.log(`📡 Default address: http://${host}:${port}`);
            console.log(`🔧 Available tools: Research, Code, Terminal, Memory`);
            console.log(
              `📚 Use 'claude-flow mcp tools' to see all available tools`,
            );
          } catch (err) {
        ...
        case "status": {
          try {
            ...
            if (health.mcp) {
              ...
              console.log(
                `🔐 Authentication: ${mcpConfig.auth ? "Enabled" : "Disabled"}`,
              );
        ...
        default: {
          ...
          console.log(
            "Available subcommands: start, stop, status, tools, config, restart, logs",
          );
          break;
        }
      }
    ...
    action: async (ctx: CommandContext) => {
      ...
      switch (subcommand) {
        case "store": {
          ...
          try {
            const namespace =
              (ctx.flags.namespace as string) ||
              (ctx.flags.n as string) ||
              "default";
            ...
            console.log(
              `💾 Size: ${new TextEncoder().encode(value).length} bytes`,
            );
          } catch (err) {
        ...
        case "query": {
          ...
          try {
            const namespace =
              (ctx.flags.namespace as string) || (ctx.flags.n as string);
            const limit =
              (ctx.flags.limit as number) || (ctx.flags.l as number) || 10;
            ...
            for (const entry of limited) {
              ...
              console.log(
                `   Stored: ${new Date(entry.timestamp).toLocaleString()}`,
              );
            }
        ...
        case "cleanup": {
          try {
            const days =
              (ctx.flags.days as number) || (ctx.flags.d as number) || 30;
            const removed = await memory.cleanup(days);
            success("Cleanup completed");
            console.log(
              `🗑️  Removed: ${removed} entries older than ${days} days`,
            );
          } catch (err) {
        ...
        default: {
          console.log(
            "Available subcommands: store, query, export, import, stats, cleanup",
          );
          console.log("\nExamples:");
          console.log(
            `  ${blue("memory store")} previous_work "Research findings from yesterday"`,
          );
...
Now, please proceed with the task: ${task}`;
            ...
            if (ctx.flags.dryRun || ctx.flags["dry-run"] || ctx.flags.d) {
              ...
              console.log(`\nEnhanced Features:`);
              console.log(
                `  - Memory Bank enabled via: npx claude-flow memory commands`,
              );
              console.log(
                `  - Coordination ${ctx.flags.parallel ? "enabled" : "disabled"}`,
              );
            ...
            console.log(`💾 Commit: ${ctx.flags.commit || "phase"}`);
            console.log(
              `✨ Enhanced with Claude-Flow guidance for memory and coordination`,
            );
            console.log("");
        ...
        case "batch": {
          ...
          try {
            ...
            for (const task of workflow.tasks) {
              ...
              // Add tools
              if (task.tools) {
                const toolsList = Array.isArray(task.tools)
                  ? task.tools.join(",")
                  : task.tools;
        ...
        default: {
          ...
          console.log(
            '  claude-flow claude spawn "fix bug in payment system" --no-permissions',
          );
  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      try {
        ...
        const interval =
          ((ctx.flags.interval as number) || (ctx.flags.i as number) || 2) *
          1000;
        const isCompact =
          (ctx.flags.compact as boolean) || (ctx.flags.c as boolean) || false;
        let running = true;
          ...
          try {
            ...
            if (agents.length > 0 && !isCompact) {
              ...
              for (const agent of agents.slice(0, 5)) {
                console.log(
                  `   • ${agent.id.substring(0, 20)}... (${agent.type}) - ${agent.status}`,
                );
              }
              if (agents.length > 5) {
            ...
            if (tasks.length > 0 && !isCompact) {
              ...
              for (const task of tasks.slice(0, 5)) {
                const assignedTo = task.assignedAgent
                  ? `→ ${task.assignedAgent.substring(0, 15)}...`
                  : "(unassigned)";
                console.log(
                  `   • ${task.id.substring(0, 20)}... (${task.type}) - ${task.status} ${assignedTo}`,
                );
              }
              if (tasks.length > 5) {
            ...
            console.log("\n" + "─".repeat(50));
            console.log(
              `Last updated: ${new Date().toLocaleTimeString()} • Interval: ${interval / 1000}s`,
            );
  ...
  cli.command({
    name: "swarm-ui",
    description:
      "Create self-orchestrating Claude agent swarms with blessed UI",
    options: [
      {
        name: "strategy",
        short: "s",
  ...
  cli.command({
    ...
    action: (ctx: CommandContext) => {
      ...
      if (command === "claude") {
        ...
        console.log();
        console.log(
          "Spawn and manage Claude Code instances with specific configurations.",
        );
        console.log();
        ...
        console.log(
          "  batch <file>    Execute multiple Claude instances from workflow",
        );
        console.log();
        ...
        console.log(
          `  ${blue("claude-flow claude batch")} workflow.json --dry-run`,
        );
        console.log();
        console.log(
        ...
        console.log();
        console.log(
          "Create self-orchestrating Claude agent swarms to tackle complex objectives.",
        );
        console.log();
        ...
        console.log(
          "  --ui                       Use blessed terminal UI (avoids TTY issues)",
        );
        console.log();
        console.log(bold("Examples:"));
        console.log(`  ${blue("claude-flow swarm")} "Build a REST API"`);
        console.log(
          `  ${blue("claude-flow swarm-ui")} "Build a REST API"  # Avoids TTY issues`,
        );
        console.log(
        ...
        console.log(
          "If you encounter 'Raw mode is not supported' errors, use:",
        );
        console.log(
          `  - ${blue("claude-flow swarm-ui")} <objective>  # Recommended`,
        );
        console.log(`  - ${blue("claude-flow swarm")} <objective> --ui`);
        console.log();
        console.log("For more information, see:");
        console.log(
          "  - https://github.com/ruvnet/claude-code-flow/docs/12-swarm.md",
        );
        console.log(
          "  - https://github.com/ruvnet/claude-code-flow/SWARM_TTY_SOLUTION.md",
        );
        ...
        console.log(bold("Options:"));
        console.log(
          "  -n, --namespace <ns>     Memory namespace for this session",
        );
...
function getDefaultPromptForType(type: string): string {
  const prompts: Record<string, string> = {
    coordinator:
      "You are a coordination agent responsible for planning and delegating tasks.",
    researcher:
      "You are a research agent specialized in gathering and analyzing information.",
    implementer:
      "You are an implementation agent focused on writing code and creating solutions.",
    analyst:
      "You are an analysis agent that identifies patterns and generates insights.",

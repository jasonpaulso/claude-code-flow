...
export const workflowCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option(
        "-v, --variables <vars:string>",
        "Override variables (JSON format)",
      )
  ...
  .command(
    ...
    new Command()
      ...
      .option("--format <format:string>", "Template format (json, yaml)", {
        default: "json",
      })
...
async function runWorkflow(workflowFile: string, options: any): Promise<void> {
  try {
    ...
    console.error(
      colors.red("Workflow execution failed:"),
...
async function validateWorkflow(
  ...
  try {
    ...
    console.error(
      colors.red("✗ Workflow validation failed:"),
      (error as Error).message,
    );
    Deno.exit(1);
  }
...
async function showWorkflowStatus(
  ...
  try {
    ...
    console.error(
      colors.red("Failed to get workflow status:"),
      (error as Error).message,
    );
  }
}
...
async function stopWorkflow(workflowId: string, options: any): Promise<void> {
  try {
    ...
    if (execution.status !== "running") {
      console.log(
        colors.yellow(`Workflow is not running (status: ${execution.status})`),
      );
      return;
    }
    ...
    console.error(
      colors.red("Failed to stop workflow:"),
      (error as Error).message,
    );
  }
}
...
async function generateTemplate(
  ...
  if (!template) {
    ...
    console.log(
      colors.gray("Available templates:"),
  ...
  const outputFile =
    options.output || `${templateType}-workflow.${options.format}`;
...
async function loadWorkflow(workflowFile: string): Promise<WorkflowDefinition> {
  try {
    ...
    throw new Error(
      `Failed to load workflow file: ${(error as Error).message}`,
    );
  }
}
    ...
    if (task.depends) {
      for (const dep of task.depends) {
        if (!taskIds.has(dep)) {
          ...
          const depExists = workflow.tasks
            .slice(0, taskIndex)
            .some((t) => t.id === dep);
          if (!depExists) {
  ...
  if (workflow.agents) {
    ...
    for (const task of workflow.tasks) {
      if (task.assignTo && !agentIds.has(task.assignTo)) {
        errors.push(
          `Task ${task.id}: assigned to unknown agent ${task.assignTo}`,
        );
      }
    }
  }
...
async function executeWorkflow(
  ...
  for (let i = 0; i < execution.tasks.length; i++) {
    ...
    if (success) {
      ...
      if (
        options.failFast ||
        workflow.settings?.failurePolicy === "fail-fast"
      ) {
  ...
  console.log(
    `${colors.white("Tasks:")} ${execution.progress.completed}/${execution.progress.total} completed`,
  );
...
async function watchWorkflowStatus(workflowId: string): Promise<void> {
  ...
  while (true) {
    try {
      ...
      console.error(
        colors.red("Error watching workflow:"),
        (error as Error).message,
      );
      break;
    }
...
function displayWorkflowStatus(execution: WorkflowExecution): void {
  ...
  console.log(
    `${colors.white("Started:")} ${execution.startedAt.toLocaleString()}`,
  );
  console.log(`${colors.white("Duration:")} ${duration}`);
  ...
  console.log(
    `${progressBar} ${execution.progress.completed}/${execution.progress.total}`,
  );

  if (execution.progress.failed > 0) {
    console.log(
      `${colors.white("Failed Tasks:")} ${colors.red(execution.progress.failed.toString())}`,
    );
  }
  console.log();
  ...
  for (const taskExec of execution.tasks) {
    ...
    const duration =
      taskExec.completedAt && taskExec.startedAt
        ? formatDuration(
            taskExec.completedAt.getTime() - taskExec.startedAt.getTime(),
          )
        : taskExec.startedAt
...
async function getWorkflowExecution(
  ...
  const workflow = workflows.find(
    (w) => w.id === workflowId || w.id.startsWith(workflowId),
  );

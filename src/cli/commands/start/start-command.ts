...
async function cleanup(
  ...
  try {
    ...
    try {
      await Deno.remove(".claude-flow.status.json");
    } catch {
      // Ignore error if file doesn't exist
    }

    console.log(colors.green.bold("✓"), "Cleanup complete");
  } catch (error) {
...
export const startCommand = new Command()
  ...
  .option(
    "--mcp-transport <transport:string>",
    ...
    try {
      ...
      // Daemon mode
        ...
        const signalHandler = async () => {
          await cleanup(processManager, systemMonitor);
          Deno.exit(0);
        };
        ...
        console.log(
          colors.gray('Use "claude-flow status" to check system status'),
        );
      ...
      // Interactive mode (default)
        ...
        while (true) {
          ...
          console.log(
            ...
            colors.green(
              `${stats.runningProcesses}/${stats.totalProcesses} processes running`,
            ),
          );
          console.log();
      ...
      console.error(
        colors.red.bold("Failed to start:"),

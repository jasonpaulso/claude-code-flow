...
export const sessionCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option("--format <format:string>", "Output format (table, json)", {
        default: "table",
      })
  ...
  .command(
    ...
    new Command()
      ...
      .option("--format <format:string>", "Export format (json, yaml)", {
        default: "json",
      })
...
async function listSessions(options: any): Promise<void> {
  try {
    ...
    console.error(
      colors.red("Failed to list sessions:"),
...
async function saveSession(
  ...
  try {
    ...
    console.error(
      colors.red("Failed to save session:"),
      (error as Error).message,
    );
  }
}
...
async function restoreSession(sessionId: string, options: any): Promise<void> {
  try {
    ...
    console.log(`${colors.white("Name:")} ${session.name}`);
    console.log(
      `${colors.white("Description:")} ${session.description || "None"}`,
    );
    console.log(`${colors.white("Agents:")} ${session.state.agents.length}`);
    console.log(`${colors.white("Tasks:")} ${session.state.tasks.length}`);
    console.log(
      `${colors.white("Created:")} ${session.createdAt.toLocaleString()}`,
    );

    // Confirmation
    if (!options.force) {
      const action = options.merge
    ...
    if (session.metadata.checksum !== expectedChecksum) {
      console.log(
        colors.yellow(
          "⚠ Warning: Session checksum mismatch. Data may be corrupted.",
        ),
      );
    ...
    console.error(
      colors.red("Failed to restore session:"),
      (error as Error).message,
    );
  }
}
...
async function deleteSession(sessionId: string, options: any): Promise<void> {
  try {
    ...
    console.error(
      colors.red("Failed to delete session:"),
      (error as Error).message,
    );
  }
}
...
async function exportSession(
  ...
  try {
    ...
    console.error(
      colors.red("Failed to export session:"),
      (error as Error).message,
    );
  }
}
...
async function importSession(inputFile: string, options: any): Promise<void> {
  try {
    ...
    console.log(
      `${colors.white("Action:")} ${options.overwrite ? "Overwritten" : "Created"}`,
    );
  } catch (error) {
    console.error(
      colors.red("Failed to import session:"),
      (error as Error).message,
    );
  }
}
...
async function showSessionInfo(sessionId: string): Promise<void> {
  try {
    ...
    console.log(
      `${colors.white("Tags:")} ${session.tags.join(", ") || "None"}`,
    );
    console.log(
      `${colors.white("Created:")} ${session.createdAt.toLocaleString()}`,
    );
    console.log(
      `${colors.white("Updated:")} ${session.updatedAt.toLocaleString()}`,
    );
    console.log();
    ...
    console.log(`${colors.white("Tasks:")} ${session.state.tasks.length}`);
    console.log(
      `${colors.white("Memory Entries:")} ${session.state.memory.length}`,
    );
    console.log();
    ...
    const integrityIcon = formatStatusIndicator(
      integrity ? "success" : "error",
    );
    console.log(
      `${colors.white("Integrity:")} ${integrityIcon} ${integrity ? "Valid" : "Corrupted"}`,
    );
    ...
    try {
      ...
      console.log(
        `${colors.white("Modified:")} ${fileInfo.mtime?.toLocaleString() || "Unknown"}`,
      );
    } catch {
    ...
    console.error(
      colors.red("Failed to show session info:"),
      (error as Error).message,
    );
  }
}
  ...
  try {
    ...
    if (options.orphaned) {
      ...
      toDelete = toDelete.filter(
        (session) => (session.metadata as any).orphaned,
      );
    }
    ...
    for (const session of toDelete) {
      ...
      console.log(
        `• ${session.name} (${colors.gray(session.id.substring(0, 8) + "...")}) - ${age} days old`,
      );
    }
    ...
    for (const session of toDelete) {
      try {
        ...
        console.error(
          colors.red(`Failed to delete ${session.name}:`),
          (error as Error).message,
        );
      }
    }
    ...
    console.error(
      colors.red("Failed to clean sessions:"),
      (error as Error).message,
    );
  }
}
    ...
    for await (const entry of Deno.readDir(SESSION_DIR)) {
      if (entry.isFile && entry.name.endsWith(".json")) {
        try {
          ...
          console.warn(
            colors.yellow(
              `Warning: Failed to load session file ${entry.name}:`,
            ),
            (error as Error).message,
          );
        }
...
async function loadSession(sessionId: string): Promise<SessionData | null> {
  const sessions = await loadAllSessions();
  return (
    sessions.find((s) => s.id === sessionId || s.id.startsWith(sessionId)) ||
    null
  );
}

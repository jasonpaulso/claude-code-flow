...
export const taskCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option("-p, --priority <priority:number>", "Task priority", {
        default: 0,
      })
      .option(
        "-d, --dependencies <deps:string>",
  ...
  .command(
    ...
    new Command()
      ...
      .action(async (options: any) => {
        console.log(
          colors.yellow("Task listing requires a running Claude-Flow instance"),
        );
      }),
  )
  .command(
    ...
    new Command()
      ...
      .action(async (options: any, taskId: string) => {
        console.log(
          colors.yellow(`Task status requires a running Claude-Flow instance`),
        );
      }),
  )
  .command(
    ...
    new Command()
      ...
      .action(async (options: any, taskId: string) => {
        console.log(
          colors.yellow(
            `Cancelling task ${taskId} requires a running Claude-Flow instance`,
          ),
        );
      }),
  )
    ...
    new Command()
      ...
      .action(async (options: any, workflowFile: string) => {
        try {
          ...
          console.error(
            colors.red("Failed to load workflow:"),

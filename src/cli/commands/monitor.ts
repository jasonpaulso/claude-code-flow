...
class Dashboard {
    ...
    while (this.running) {
      try {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, this.options.interval * 1000),
        );
      } catch (error) {
  ...
  private renderHeader(data: MonitorData): void {
    ...
    console.log(
      colors.cyan.bold("Claude-Flow Live Monitor") + colors.gray(` - ${time}`),
    );
  ...
  private renderFocusedComponent(
    data: MonitorData,
  ...
  private renderFooter(): void {
    console.log("─".repeat(80));
    console.log(
      colors.gray("Press Ctrl+C to exit • Update interval: ") +

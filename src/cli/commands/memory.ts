...
export class SimpleMemoryManager {
  ...
  async cleanup(daysOld: number = 30) {
    ...
    for (const namespace of Object.keys(this.data)) {
      ...
      this.data[namespace] = this.data[namespace].filter(
...
export const memoryCommand = new Command()
  ...
  .command(
    ...
    new Command()
      ...
      .option("-n, --namespace <namespace:string>", "Target namespace", {
        default: "default",
      })
  ...
  .command(
    "stats",
    new Command().description("Show memory statistics").action(async () => {
      try {
        const memory = new SimpleMemoryManager();
        const stats = await memory.getStats();
        ...
        console.error(
          colors.red("Failed to get stats:"),
          (error as Error).message,
        );
      }
    }),
  ...
  .command(
    ...
    new Command()
      ...
      .option("-d, --days <days:number>", "Entries older than n days", {
        default: 30,
      })
      .action(async (options: any) => {
        try {
          ...
          console.log(
            `🗑️  Removed: ${removed} entries older than ${options.days} days`,
          );
        } catch (error) {
          console.error(
            colors.red("Failed to cleanup:"),
            (error as Error).message,
          );
        }
      }),

...
export class CoordinationMetricsCollector {
  ...
  getCurrentMetrics(): CoordinationMetrics {
    ...
    const taskCompletions = recentSamples.filter(
      (s) => s.metric === "task.completed",
    ).length;
    ...
    return {
      ...
      performanceMetrics: {
        coordinationLatency: this.average(
          this.histograms.coordinationLatencies,
        ),
  ...
  getMetricHistory(metric: string, since?: Date): MetricsSample[] {
    ...
    return this.samples.filter(
      (s) => s.metric === metric && s.timestamp >= cutoff,
    );
  }
  ...
  getTopMetrics(
    limit = 10,
  ): Array<{ metric: string; value: number; timestamp: Date }> {
    ...
    for (const sample of recent) {
      byMetric.set(
        sample.metric,
        (byMetric.get(sample.metric) || 0) + sample.value,
      );
  ...
  private setupEventHandlers(): void {
    ...
    this.eventBus.on(SystemEvents.RESOURCE_RELEASED, (data: any) => {
      ...
      this.gauges.lockedResources = Math.max(
        0,
        this.gauges.lockedResources - 1,
      );
  ...
  private calculateResourceUtilization(): number {
    const totalResources =
      this.gauges.lockedResources + this.gauges.freeResources;

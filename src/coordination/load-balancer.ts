...
export class LoadBalancer extends EventEmitter {
  ...
  private loadHistory = new Map<
    string,
    Array<{ timestamp: Date; load: number }>
  >();
  ...
  async selectAgent(
    ...
    try {
      ...
      let candidates = this.filterAgentsByConstraints(
        availableAgents,
        task,
        constraints,
      );
  ...
  private filterAgentsByConstraints(
    ...
    return agents.filter((agent) => {
      ...
      if (
        constraints?.excludeAgents?.some(
          (excluded) => excluded.id === agent.id.id,
        )
      ) {
        return false;
      }
      ...
      if (
        constraints?.maxLoad &&
        load &&
        load.utilization > constraints.maxLoad
      ) {
        return false;
      }
  ...
  private async applySelectionStrategy(
    ...
    for (const agent of candidates) {
      ...
      const load =
        this.agentLoads.get(agentId) || this.createDefaultLoad(agentId);
  ...
  private calculatePerformanceScore(
    ...
    const throughputScore = Math.min(
      1,
      load.throughput / baseline.expectedThroughput,
    );
    const efficiencyScore = load.efficiency;
    const responseScore = Math.min(
      1,
      baseline.expectedResponseTime / load.averageResponseTime,
    );
  ...
  private calculateCapabilityScore(
    ...
    if (task.context.framework) {
      const hasFramework = agent.capabilities.frameworks.includes(
    ...
    const domainMatch = agent.capabilities.domains.some(
      (domain) =>
        task.type.includes(domain) ||
        task.requirements.capabilities.includes(domain),
    );
  ...
  private async performRebalancing(): Promise<void> {
    ...
    try {
      ...
      const overloaded = loads.filter(
        ([_, load]) =>
          load.utilization > 0.8 &&
          load.queueDepth > this.config.queueDepthThreshold,
      );
      ...
      for (const [overloadedId, overloadedLoad] of overloaded) {
        ...
        const target = underloaded.sort(
          (a, b) => a[1].utilization - b[1].utilization,
        )[0];
        ...
        if (target) {
          ...
          if (tasksToSteal > 0) {
            await this.executeWorkStealing(
              overloadedId,
              targetId,
              tasksToSteal,
            );
  ...
  private updatePerformanceBaseline(agentId: string, metrics: any): void {
    ...
    baseline.expectedResponseTime =
      baseline.expectedResponseTime * (1 - alpha) +
  ...
  getLoadStatistics(): {
    ...
    const avgUtilization =
      loads.reduce((sum, load) => sum + load.utilization, 0) / loads.length ||
      0;
...
class LoadPredictor {
  ...
  predict(task: TaskDefinition): LoadPrediction {
    const currentLoad =
      this.history.length > 0 ? this.history[this.history.length - 1].load : 0;
  ...
  private estimateTaskComplexity(task: TaskDefinition): number {
    ...
    if (
      task.requirements.estimatedDuration &&
      task.requirements.estimatedDuration > 300000
    ) {
      complexity += 0.3; // Long-running task
    }

    if (
      task.requirements.memoryRequired &&
      task.requirements.memoryRequired > 512 * 1024 * 1024
    ) {

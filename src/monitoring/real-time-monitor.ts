...
export class RealTimeMonitor extends EventEmitter {
  ...
  private calculateSwarmEfficiency(agents: AgentMetrics[]): number {
    ...
    const totalTasks = agents.reduce(
      (sum, agent) =>
        sum + (agent.tasksCompleted || 0) + (agent.tasksFailed || 0),
      0,
    );
  ...
  private calculateSwarmReliability(agents: AgentMetrics[]): number {
    ...
    const totalReliability = agents.reduce(
      (sum, agent) => sum + (agent.successRate || 1),
      0,
    );
  ...
  private calculateAverageQuality(agents: AgentMetrics[]): number {
    ...
    const totalQuality = agents.reduce(
      (sum, agent) => sum + (agent.codeQuality || 0.8),
      0,
    );
  ...
  private initializeHealthChecks(): void {
    ...
    this.healthChecks.set("system", {
      ...
      customCheck: async () => {
        ...
        return (
          this.systemMetrics.cpuUsage < 95 &&
          this.systemMetrics.memoryUsage < 95
        );
      },
    });
  }

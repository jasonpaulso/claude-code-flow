...
const terminalWriteEmulators = new Map<
  vscode.Terminal,
  vscode.EventEmitter<string>
>();

/**
 * Initialize the VSCode terminal bridge
 */
export function initializeTerminalBridge(
  ...
  (vscode.window as any).createTerminal = function (
    options: vscode.TerminalOptions,
  ) {
    const terminal = originalCreateTerminal.call(
      vscode.window,
      options,
    ) as vscode.Terminal;
 ...
 */
function setupTerminalRenderer(
  terminal: vscode.Terminal,
  terminalId: string,
): void {
 ...
 * Get terminal by ID
 */
export function getTerminalById(
  terminalId: string,

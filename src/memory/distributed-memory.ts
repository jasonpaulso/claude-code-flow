...
export class DistributedMemorySystem extends EventEmitter {
  ...
  async deletePartition(partitionId: string): Promise<void> {
    ...
    // Update local node
    const localNode = this.nodes.get(this.localNodeId)!;
    localNode.partitions = localNode.partitions.filter(
      (p) => p !== partitionId,
    );
  ...
  async store(
    ...
    try {
      ...
      // Determine partition
      const partitionId =
        options.partition || this.selectPartition(options.type || "knowledge");
      const partition = this.partitions.get(partitionId);
  ...
  private getCachedEntry(
    key: string,
  ): { entry: MemoryEntry; expiry: number } | null {
  ...
  private async syncPartitionCreation(
    partition: MemoryPartition,
  ): Promise<void> {

...
export class AgentManager extends EventEmitter {
  ...
  private scalingOperations = new Map<
    string,
  ...
  private setupEventHandlers(): void {
    this.eventBus.on("agent:heartbeat", (data: unknown) => {
      this.handleHeartbeat(data as AgentHeartbeatEvent);
    });

    this.eventBus.on("agent:error", (data: unknown) => {
      this.handleAgentError(data as AgentErrorEvent);
    });

    this.eventBus.on("task:assigned", (data: unknown) => {
      this.updateAgentWorkload((data as TaskAssignedEvent).agentId, 1);
    });

    this.eventBus.on("task:completed", (data: unknown) => {
      this.updateAgentWorkload((data as TaskCompletedEvent).agentId, -1);
      this.updateAgentMetrics(
        (data as TaskCompletedEvent).agentId,
        (data as TaskCompletedEvent).metrics as AgentMetrics
      );
    });

    this.eventBus.on("resource:usage", (data: unknown) => {
      this.updateResourceUsage(
        (data as ResourceUsageEvent).agentId,
        (data as ResourceUsageEvent).usage as {
          cpu: number;
          memory: number;
          disk: number;
        }
  ...
  private spawnAgentProcess(agent: AgentState): ChildProcess {
    const runtime = agent.environment.runtime as string;
    const availableTools = agent.environment.availableTools;
    const config = agent.config;
    const workingDirectory = agent.environment.workingDirectory;
    const args = [
      "run",
      "--allow-all",
      availableTools[0] || "./agents/generic-agent.ts",
      "--config",
    ...
    return new Promise((resolve, reject) => {
      ...
      const handler = (data: { agentId: string }): void => {
        if (data.agentId === agentId) {
          clearTimeout(timer);
          this.eventBus.off("agent:ready", handler as any);
          resolve();
        }
      };

      this.eventBus.on("agent:ready", handler as any);
    });
  }
  ...
  private async waitForProcessExit(
    ...
    return new Promise((resolve) => {
      const childProcess = this.processes.get(agentId) as
        | ChildProcess
        | undefined;
      if (!childProcess || childProcess.killed) {
        resolve();
  ...
  private handleHeartbeat(data: {
    ...
    const agent = this.agents.get(data.agentId) as AgentState | undefined;
    if (!agent) return;

    agent.lastHeartbeat = data.timestamp;

    if (data.metrics) {
      this.updateAgentMetrics(data.agentId, data.metrics as AgentMetrics);
    }
  ...
  private updateAgentStatus(agentId: string, status: AgentStatus): void {
    const agent = this.agents.get(agentId) as AgentState | undefined;
    if (!agent) return;

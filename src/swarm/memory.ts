...
export class SwarmMemoryManager extends EventEmitter {
  ...
  async store(
    ...
    if (entrySize > this.config.maxEntrySize) {
      throw new Error(
        `Entry size ${entrySize} exceeds maximum ${this.config.maxEntrySize}`,
      );
    }
  ...
  async retrieve(
    ...
    if (this.config.enableCaching) {
      ...
      if (cached && !this.isExpired(cached)) {
        ...
        return options.includeMetadata
          ? cached
          : await this.deserializeValue(cached.value);
      }
    }
    ...
    return options.includeMetadata
      ? entry
      : await this.deserializeValue(entry.value);
  }
  ...
  async query(query: MemoryQuery): Promise<MemoryEntry[]> {
    ...
    if (query.expiresAfter) {
      results = results.filter(
        (e) => e.expiresAt && e.expiresAt >= query.expiresAfter!,
      );
    }
  ...
  async shareMemory(
    ...
    const targetPartition = await this.getOrCreatePartition(
      `agent_${targetAgent.id}`,
    );
  ...
  async deletePartition(
    ...
    if (partition.entries.length > 0 && !force) {
      throw new Error(
        `Partition ${name} contains entries. Use force=true to delete.`,
      );
    }
    ...
    this.memory.partitions = this.memory.partitions.filter(
      (p) => p.id !== partition.id,
    );
  ...
  getStatistics(): MemoryStatistics {
    ...
    for (const entry of validEntries) {
      ...
      if (
        entry.expiresAt &&
        entry.expiresAt.getTime() - Date.now() < 24 * 60 * 60 * 1000
      ) {
        expiringEntries++;
      }
    }
    ...
    return {
      ...
      entriesByAccess,
      averageSize:
        validEntries.length > 0 ? totalSize / validEntries.length : 0,
      oldestEntry,
      newestEntry,
      expiringEntries,
    };
  ...
  private async cleanupExpiredEntries(): Promise<void> {
    const expiredEntries = Array.from(this.entries.values()).filter((e) =>
      this.isExpired(e),
    );
...
class MemoryCache {
  private cache: Map<string, { entry: MemoryEntry; expiry: number }> =
    new Map();

...
export class TaskExecutor extends EventEmitter {
  ...
  async shutdown(): Promise<void> {
    ...
    const stopPromises = Array.from(this.activeExecutions.values()).map(
      (session) => this.stopExecution(session.id, "Executor shutdown"),
    );
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        session.stop("Timeout").then(() => {
          reject(
            new Error(
              `Execution timed out after ${session.config.timeoutMs}ms`,
            ),
          );
        });
        ...
        process.on(
          ...
          async (code: number | null, signal: string | null) => {
            ...
            try {
              ...
              if (isTimeout) {
                reject(
                  new Error(`Claude execution timed out after ${timeout}ms`),
                );
              } else if (exitCode !== 0) {
                reject(
                  new Error(
                    `Claude execution failed with exit code ${exitCode}: ${errorBuffer}`,
                  ),
                );
              } else {
                resolve(result);
  ...
  private buildClaudePrompt(task: TaskDefinition, agent: AgentState): string {
    ...
    sections.push(
      `You are ${agent.name}, a ${agent.type} agent in a swarm system.`,
    );
    ...
    sections.push(
      `- Quality threshold: ${task.requirements.minReliability || 0.8}`,
    );
    ...
    sections.push(
      "6. When complete, provide a clear summary of what was accomplished",
    );
    sections.push("");
    ...
    return {
      ...
      resources: {
        maxMemory:
          ...
          SWARM_CONSTANTS.DEFAULT_MEMORY_LIMIT,
        maxCpuTime:
          task.requirements.maxDuration || SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
    ...
    this.resourceMonitor.on("limit-violation", (data: any) => {
      ...
      if (session) {
        session.stop("Resource limit violation").catch((error) => {
          this.logger.error(
            "Error stopping session due to resource violation",
            {
...
class ProcessPool extends EventEmitter {
  ...
  getSuccessRate(): number {
    return this.totalExecutions > 0
      ? this.successCount / this.totalExecutions
      : 0;
  }

...
describe("Product Endpoints", () => {
  ...
  describe("GET /api/products", () => {
    ...
    it("should filter products by category", async () => {
      ...
      expect(response.body.data).toHaveLength(3);
      expect(
        response.body.data.every((p) => p.category === "Electronics"),
      ).toBe(true);
    });
    ...
    it("should sort products", async () => {
      ...
      expect(response.body.data[0].name).toBe("Laptop");
      expect(response.body.data[response.body.data.length - 1].name).toBe(
        "Mouse",
      );
    });
  ...
  describe("Special Product Endpoints", () => {
    describe("GET /api/products/featured", () => {
      it("should get featured products", async () => {
        ...
        expect(
          response.body.data.every((p) => p.tags.includes("featured")),
        ).toBe(true);
      });
    });

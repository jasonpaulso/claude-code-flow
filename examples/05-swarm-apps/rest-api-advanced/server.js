...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "A production-ready REST API with authentication, validation, and best practices",
      contact: {
        name: "API Support",
...
async function startServer() {
  try {
    ...
    server = app.listen(PORT, () => {
      ...
      logger.info(
        `API Documentation available at ${process.env.API_URL || "http://localhost:" + PORT}/api-docs`,
      );
    });
  } catch (error) {

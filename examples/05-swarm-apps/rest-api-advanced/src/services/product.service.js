...
class ProductService {
  ...
  static async calculateDynamicPrice(productId) {
    ...
    const dynamicPrice =
      Math.round(product.price * priceMultiplier * 100) / 100;
  ...
  static async getLowStockProducts(threshold) {
    const products = await Product.find({
      ...
      .select(
        "name sku inventory.quantity inventory.lowStockThreshold category",
      );
  ...
  static async getProductAnalytics(productId, period = 30) {
    ...
    if (product.rating.average < 3) {
      analytics.recommendations.push(
        "Consider improving product quality based on reviews",
      );
    }

    if (product.views > 1000 && product.salesCount < 10) {
      analytics.recommendations.push(
        "High views but low conversion - consider adjusting price or description",
      );
    }

...
const createOrderValidation = [
  ...
  body("discountCode").optional().trim().toUpperCase(),
...
const addTrackingValidation = [
  ...
  body("number").trim().notEmpty().withMessage("Tracking number is required"),

  body("url").optional().trim().isURL().withMessage("Invalid tracking URL"),
...
const searchOrdersValidation = [
  ...
  query("trackingNumber").optional().trim(),
  ...
  query("sort")
    .optional()
    .matches(/^-?(createdAt|totalAmount|status)$/)

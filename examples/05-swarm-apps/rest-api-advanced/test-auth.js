...
async function testPasswordStrength() {
  ...
  for (const test of passwords) {
    try {
      const result = await apiCall("POST", "/auth/check-password", {
        password: test.password,
      });
      console.log(
        `✓ Password "${test.password}": ${result.data.level} (score: ${result.data.score}/5)`,
      );
    } catch (error) {
  ...
  for (let i = 1; i <= 6; i++) {
    try {
      ...
      if (i < 5) {
        ...
        console.log(
          `✓ Login attempt ${i}: Failed and account should be locked`,
        );
      } else {

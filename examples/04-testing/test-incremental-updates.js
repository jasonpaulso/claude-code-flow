...
async function testSwarmMemoryVersions() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    console.log(
      `  ✓ Update ${i}: value=${current.value + 1}, version=${current.version + 1}`,
    );
  }

  const final = await memory.get("counter");
  console.log(
    `  📊 Final state: value=${final.value}, version=${final.version}`,
  );
...
async function testConcurrentCounters() {
  ...
  console.log(
    `  ✓ ${numAgents} agents completed ${incrementsPerAgent} increments each`,
  );
  console.log(
    `  📊 Final counter: value=${final.value}, version=${final.version}`,
  );
  console.log(
    `  ✓ Expected: ${numAgents * incrementsPerAgent}, Actual: ${final.value}`,
  );

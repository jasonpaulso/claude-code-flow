...
for (let i = 0; i < 5; i++) {
  ...
  store.update("config", {
    ...current.value,
    fontSize: current.value.fontSize + 1,
  });
  console.log(
    `  ✓ Update ${i + 1}: fontSize=${current.value.fontSize + 1}, version=${current.version + 1}`,
  );
}

const final = store.get("config");
console.log(
  `  📊 Final: version=${final.version}, history_length=${final.history.length}`,
);

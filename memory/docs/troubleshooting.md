...
const specific = await memory.query({
  ...
  dateRange: {
    // Use date range instead of full scan
...
async function debugVectorSearch(query: string): Promise<void> {
  ...
  const stats = await memory.getStatistics();
  console.log(
    `Total items with embeddings: ${stats.embeddingStats.totalEmbeddings}`,
  );

  if (stats.embeddingStats.totalEmbeddings === 0) {
    console.log(
      "No embeddings found. Generating embeddings for existing items...",
    );
  ...
  for (const threshold of thresholds) {
    ...
    if (results.length > 0) {
      console.log(
        `Best match: ${results[0].similarity.toFixed(3)} - ${results[0].item.content.substring(0, 100)}...`,
      );
    }
  }
}
...
async function validateEmbeddings(): Promise<void> {
  ...
  if (sampleItem.length > 0 && sampleItem[0].embedding) {
    ...
    if (inconsistentItems.length > 0) {
      console.warn(
        `Found ${inconsistentItems.length} items with inconsistent embedding dimensions`,
      );
    }
  }
}
...
class PerformanceProfiler {
  ...
  printProfileReport(): void {
    ...
    console.log(
      `Average duration: ${report.summary.avgDuration.toFixed(2)}ms\n`,
    );
    ...
    report.profiles.slice(0, 10).forEach((profile, index) => {
      ...
      console.log(
        `   Avg: ${profile.avgDuration.toFixed(2)}ms, Count: ${profile.count}, Errors: ${profile.errorRate.toFixed(2)}%`,
      );
    });
  }
}

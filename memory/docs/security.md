  ...
  async generateToken(agentId: string, options: TokenOptions): Promise<string> {
    const payload: JWTPayload = {
      ...
      exp:
        ...
        this.parseExpiration(this.config!.expiresIn),
      roles: options.roles || [],
...
class NamespaceSecurityManager {
  ...
  private async hasNamespaceAccess(
    ...
    if (!permissions) {
      ...
      return this.hasNamespaceAccess(
        auth,
  ...
  private restrictQueryToNamespaces(
    query: MemoryQuery,
    auth: AuthContext,
  ): void {
    ...
    if (query.namespace) {
      ...
      query.namespaces = query.namespaces.filter((ns) =>
        allowedNamespaces.includes(ns),
      );
    } else {
  ...
  private getAllowedNamespaces(auth: AuthContext): string[] {
    ...
    if (this.config.permissions) {
      for (const [namespace, permissions] of Object.entries(
...
class IntegrityManager {
  ...
  async verifyItem(item: MemoryItem): Promise<IntegrityResult> {
    ...
    if (item.checksum && this.config?.verifyOnRead) {
      const contentValid = await this.verifyChecksum(
        item.content,
        item.checksum,
      );
      if (!contentValid) {
  ...
  private verifyMetadataIntegrity(item: MemoryItem): boolean {
    ...
    for (const field of requiredFields) {
      if (
        !(field in item) ||
        item[field as keyof MemoryItem] === null ||
        item[field as keyof MemoryItem] === undefined
      ) {
        return false;
      }
    }
...
class SecureDeletion {
  ...
  private async overwriteData(
    ...
    for (let pass = 0; pass < overwritePasses; pass++) {
      const randomContent = crypto
        .randomBytes(item.content.length)
        .toString("base64");
  ...
  private async logSecureDeletion(
    itemId: string,
    item: MemoryItem,
  ): Promise<void> {
    const auditEvent = {
...
class AuditLogger {
  ...
  private determineLogLevel(event: AuditEvent): "info" | "warning" | "error" {
    if (
      event.operation.includes("delete") ||
...
class RateLimiter {
  ...
  async checkRateLimit(
    ...
    if (!allowed) {
      ...
      console.warn(
        `Rate limit exceeded for agent ${agentId}, operation ${operation}`,
      );
    }
...
class InputValidator {
  static validateMemoryItem(item: Partial<MemoryItem>): ValidationResult {
    ...
    if (item.content && item.content.length > 1024 * 1024) {
      // 1MB limit

...
export class MemoryManager extends EventEmitter {
  ...
  async store(
    ...
    // Check for conflicts
    const existingItem = await this.backend.get(
      fullItem.category,
      fullItem.key
    );
    if (existingItem) {
      const resolved = await this.conflictResolution.resolve(
        existingItem,
        fullItem
      );
    ...
    // Update index
    await this.indexer.indexItem(fullItem);
  ...
  async query(query: MemoryQuery): Promise<MemoryItem[]> {
    ...
    if (query.namespace && this.namespaceManager) {
      results = results.filter(
        (item) => item.metadata?.namespace === query.namespace
      );
    }
  ...
  async import(
    ...
    for (const item of snapshot.items) {
      ...
      if (options?.merge) {
        ...
        await this.indexer.indexItem(item);
      }
  ...
  async getStats(): Promise<any> {
    ...
    return {
      ...
      replication: this.replicationManager
        ? await this.replicationManager.getStats()
        : null,
    };
  }
  ...
  private exportToMarkdown(items: MemoryItem[]): string {
    ...
    for (const [category, categoryItems] of Object.entries(byCategory)) {
      ...
      for (const item of categoryItems) {
        markdown += `### ${item.key}\n`;
        markdown += `- ID: ${item.id}\n`;
        markdown += `- Timestamp: ${new Date(
          item.metadata?.timestamp || 0
        ).toISOString()}\n`;
        markdown += `- Value:\n\`\`\`json\n${JSON.stringify(
          item.value,
          null,
...
class CRDTConflictResolution implements ConflictResolution {
  ...
  private mergeItems(existing: MemoryItem, incoming: MemoryItem): MemoryItem {
    ...
    merged.metadata = {
      ...
      mergedAt: Date.now(),
      mergedFrom: [
        existing.metadata?.nodeId || "",
        incoming.metadata?.nodeId || "",
      ],
    };

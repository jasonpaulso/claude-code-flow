...
const backends = [
  {
    name: "SQLite",
    createBackend: (dir: string) =>
      new SqliteBackend({ path: path.join(dir, "test.db") }),
  },
  {
    name: "Markdown",
    createBackend: (dir: string) =>
      new MarkdownBackend({ rootPath: dir, prettyPrint: true }),
  },
];
  ...
  describe(`${name} Backend`, () => {
    ...
    describe("Query Operations", () => {
      ...
      it("should query by namespace", async () => {
        ...
        expect(results).toHaveLength(3);
        expect(
          results.every((item) => item.metadata?.namespace === "app1"),
        ).toBe(true);
      });

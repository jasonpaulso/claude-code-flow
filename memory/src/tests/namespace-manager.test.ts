...
describe("NamespaceManager", () => {
  ...
  describe("Namespace CRUD Operations", () => {
    ...
    it("should prevent deletion of default namespace", async () => {
      await expect(namespaceManager.deleteNamespace("default")).rejects.toThrow(
  ...
  describe("Namespace Operations", () => {
    ...
    it("should merge namespaces with skip conflict resolution", async () => {
      ...
      expect(targetItems.find((i) => i.key === "shared-key")?.value).toBe(
        "target value",
      );
    });
    ...
    it("should merge namespaces with rename conflict resolution", async () => {
      ...
      expect(targetItems.find((i) => i.key === "conflict")?.value).toBe(
        "target value",
      );
      expect(
        targetItems.find((i) => i.key === `conflict_${source.id}`)?.value,
      ).toBe("source value");
    });
  });

...
describe("MemoryManager", () => {
  ...
  describe("Store and Retrieve", () => {
    ...
    it("should handle namespaces", async () => {
      ...
      const item1 = await memoryManager.get(
        "test",
        "namespace-test",
        "namespace1",
      );
      const item2 = await memoryManager.get(
        "test",
        "namespace-test",
        "namespace2",
      );
  ...
  describe("Query", () => {
    ...
    it("should query by tags", async () => {
      ...
      expect(results).toHaveLength(2);
      expect(
        results.every((item) => item.metadata?.tags?.includes("active")),
      ).toBe(true);
    });
      ...
      expect(
        results.every((item) =>
          item.metadata?.timestamp
            ? item.metadata.timestamp <= beforeTime
            : true,
        ),
      ).toBe(true);
    });
